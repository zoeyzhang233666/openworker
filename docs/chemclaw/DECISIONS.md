...
- **D-167（2026-08-19）**：**内置 chem-data-hub / chem-biz-scope MCP（B1）**：安装包可携带混淆后的密钥 bundle（构建机旁路，不进 Git）；首次启动种入全局 `mcp.json`（仅 `${VAR}`）与状态目录 `.env`；连接页只读「内置」（可启停，不可删/改 url·headers/粘贴覆盖）。用户已自配同名 server 不覆盖；无 bundle/无 token 不种假条目。混淆≠对外密钥保管，反编译/抓包仍可能提取。规格/计划：`docs/superpowers/specs/2026-08-19-chemclaw-builtin-mcp-design.md` / `docs/superpowers/plans/2026-08-19-chemclaw-builtin-mcp.md`。
