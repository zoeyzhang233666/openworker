...
- **D-166（2026-08-19）**：**化工现货与期货路由纠偏**：市场口径优先于品种名；明确“现货/spot”只使用 chem-data-hub MCP `get_price_trend`，明确国内期货才使用 `lookup_cn_futures_*`，WTI/Brent 期货使用 Yahoo OHLC。甲醇、原油等同时存在现货/期货口径的裸问价先 `ask_user` 澄清；澄清前禁止行情 Tool，回答后只放行所选口径。纯查价不暴露 Web；新闻/驱动可补 Web，但不得替代价格。chem-data-hub 缺失或空结果写 unavailable，不以网页、Yahoo 或期货补价。`TurnEngine.run()`、权限审批、流式和 ChartSpec 不变。规格/计划：`docs/superpowers/specs/2026-08-19-chemclaw-spot-futures-routing-design.md` / `docs/superpowers/plans/2026-08-19-chemclaw-spot-futures-routing.md`。
