...
- **D-006**：V1 一级导航为对话、技能、智能体、定时任务、连接和设置；审批、审计与历史为二级页面。（2026-08-05：品类名由「专家龙虾」改为「智能体」；角色显示名仍可含「龙虾」。）（2026-08-09：**D-098** 曾在智能体与定时任务之间增加「客户清单」。）（2026-08-19：**D-168** 退役 GUI 侧栏「客户清单」页，主导航恢复为本条原文六项。）
...
- **D-180（2026-08-21）**：第一方对模型可见的 system / guidance / 默认核心工具 schema `description` 默认使用简体中文，并显式要求「用简体中文思考与回复」；工具 **name** 与 JSON 参数键保持英文以保证 function calling。GUI 仍原样展示上游 `reasoning`（不伪造、不机器翻译）。本刀不汉化全部 bundled Skill 正文，也不按 UI locale 维护两套 prompt。
- **D-182（2026-08-21）**：研究路径剩余英文提示纠偏——汉化 `start_subagent`/`background_task_*`/`explore` docstring、技能目录头与 `load_skill`/`search_skills`、`<market-scope-policy>`、以及 explore/research/worker 子智能体 Profile instructions（含「用简体中文思考与回复」）。不翻译 ThinkingBlock；不整库 Skill；不改 MCP 厂商 schema。须重启 sidecar 后复验。
...
- **D-185（2026-08-21）**：对话与 Markdown 产物预览共用 `Markdown.tsx`，支持 `$…$` / `$$…$$` LaTeX（`remark-math` + `rehype-katex` + 直依赖 `katex`）；坏公式不炸整段；不成对货币 `$100` 保持文本。不改 Agent prompt、ThinkingBlock、HTML 网页版注入。
...
- **D-098（2026-08-09）**：用户批准销售主线队列后实现**客户清单工作台首包**：平台 Tool `format_lead_list`（`coworker/leads/`）+ Skill `chem-lead-list`；挂外贸/内贸拓客龙虾；GUI 主导航增加「客户清单」（修订 D-006），支持导入 LeadList JSON、导出 CSV、本地标记可联系/待补查/排除与备注；**不**显示发送/CRM 按钮。SMTP 与 TED/Comtrade/国内登记仅落盘后续计划，不在本决策内实现。计划见 `docs/superpowers/plans/2026-08-09-chemclaw-lead-list-workbench.md`。（2026-08-19 **D-168**：GUI 侧栏工作台已退役；Tool/Skill/对话产物仍有效。）
...
- **D-102（2026-08-09）**：用户点名实现**客户清单进阶 UX**：`LeadsWorkbench` 行展开（匹配原因/关键证据/下一步/排除原因）；「继续补查」「调整 ICP 并重评」经 `requestLeadFollowup` 注入对话意图并切回会话；导入 JSON 若含 `run_id`/`stage`/`budget` 显示断点只读条；**仍无**发送邮件与 CRM 按钮；不静默改分、不自动重跑拓客。计划见 `docs/superpowers/plans/2026-08-09-chemclaw-lead-list-advanced-ux.md`。（2026-08-19 **D-168**：该 GUI 进阶 UX 随侧栏页一并退役。）
...
- **D-161（2026-08-18）**：**ApiHub CN 带工具真流式单独验证**：known-safe 增加精确 `(hostname, model)` 对表，不认「兼容 OpenAI」。Live 探针（`CHEMCLAW_LIVE_APIHUB_CN=1`，读产品 `%APPDATA%\\ChemClaw\\secrets.json`）对画廊四模型各 N=3。结果：**无 PASS**，`_KNOWN_SAFE_COMPAT_PAIRS` 保持空集，生产默认仍 `compat_buffered`。`deepseek-v4-flash` / `deepseek-v4-pro` / `glm-5.2`：结构化 `echo_probe` 与 `reasoning_delta` 均到达，salvage 未触发，无 progress 后断流；综合轮多 chunk 但首末 `text_delta` 间隔约 60–90ms，未达 ≥200ms 门槛（`answer_burst`）。`kimi-k3`：网关 400「价格尚未由管理员配置」。Intl / 自定义 host 仍排除。不改 Fast Router / 龙虾等待 / 思考块收起。夹具：`docs/chemclaw/fixtures/apihub_cn_stream_capability.json`。
- **D-162（2026-08-18）**：**产物 Markdown 短引用回查出图**：报告 `.md` 里的 ````chart` `from_tool`+`symbol` 与对话共用本会话 OHLC 工具回查（`yahooChartToolsFromItems` → RightRail 预览）。不再因产物预览未传 `chartToolResults` 报 `no … result in this session`。不手抄 OHLC、不写 sidecar、会话外打开该文件仍无工具上下文。guidance 写明产物短引用同样回查本会话。
- **D-163（2026-08-18）**：**ApiHub CN 长答再验**：D-161 综合轮改为 ≥12 句/≥400 字中文简报，**不放宽** ≥200ms。Live N=3（~438s）：`deepseek-v4-pro` / `glm-5.2` **PASS**（长答增量间隔约 2.8–8.0s）写入 `_KNOWN_SAFE_COMPAT_PAIRS`；`deepseek-v4-flash` **FAIL**（两轮长答已流，第 3 次工具轮超时）；`kimi-k3` **FAIL**（超时 + 400 未定价）。Kill switch OFF、Intl、自定义 host、flash 仍 buffered。不改 Fast Router / 龙虾等待 / 思考块收起。夹具：`docs/chemclaw/fixtures/apihub_cn_stream_capability.json`。
- **D-164（2026-08-18）**：**ApiHub CN Flash 单独重跑**：同一长答协议只测 `deepseek-v4-flash`，N=3，不放宽 200ms。Live ~39s **PASS**（三轮长答间隔约 4.7–5.1s），`(apihub.chem-cloud.cn, deepseek-v4-flash)` 写入对表。默认对话模型因此可走带工具真流式。Pro/GLM 不重跑，维持 D-163 PASS；kimi-k3 仍排除。不改 Fast Router / 思考块 / Intl。
- **D-165（2026-08-19）**：**按需上下文与 ApiHub Flash 真流式修复**：真实会话每轮经单一 `TurnPlanner -> TurnPlan` seam 应用 Router/Profile；FAST/KNOWLEDGE 不发送工具 schema 与 Skill catalog，prompt 按 profile 投影并有独立 kill switch；明确工作区/Memory/调度/消息/Skill/销售动作使用能力包，不确定、pending、附件、默认/强制 Skill、非默认 Persona、后台任务与未知 MCP/connector 保守回退完整 Agent。`reasoning_mode` 只控制 Provider 请求，`show_reasoning` 独立且默认有则实时显示。新会话以 `_prompt_policy_version=1` 标记，旧会话完整 legacy；`search_skills` 只返回元数据，`load_skill` 仍读最新正文。`prompt_projection_enabled` built-in = 候选 ON。真实 ApiHub GUI/WebSocket N=5 + 长答 PASS：问候 85 prompt tokens、tools/skills=0、direct 无 fallback、TTFT P50=2.88s/P95=6.88s；长答 745 个正文 delta。上游未返回 reasoning，ChemClaw 不伪造。不开启 DeepSeek Harness，不把主控/子 Agent 设为默认架构。规格/计划：`docs/superpowers/specs/2026-08-19-chemclaw-on-demand-context-streaming-design.md` / `docs/superpowers/plans/2026-08-19-chemclaw-on-demand-context-streaming.md`。
- **D-166（2026-08-19）**：**化工现货与期货路由纠偏**：市场口径优先于品种名；明确“现货/spot”只使用 chem-data-hub MCP `get_price_trend`，明确国内期货才使用 `lookup_cn_futures_*`，WTI/Brent 期货使用 Yahoo OHLC。甲醇、原油等同时存在现货/期货口径的裸问价先 `ask_user` 澄清；澄清前禁止行情 Tool，回答后只放行所选口径。**默认单口径**；显式期现对照例外见 D-177。**Web 条款已由 D-181 修订**：化工路径允许 Web 有序降级；仍禁止用 Web/Yahoo/期货交叉冒充另一口径的结构化价。`TurnEngine.run()`、权限审批、流式和 ChartSpec 不变。规格/计划：`docs/superpowers/specs/2026-08-19-chemclaw-spot-futures-routing-design.md` / `docs/superpowers/plans/2026-08-19-chemclaw-spot-futures-routing.md`。
- **D-192（2026-08-27）**：**Channel 后台投递补挂 MCP**。`deliver_to_session`（企业微信/Slack/Telegram 私信与订阅、self-wake）在跑轮次前调用与 GUI WebSocket 相同的 `prepare_mcp_tools`，通过 `_prepare_engine_for_turn()` 统一路径；修复 Harness 报「已配置但未连接」且 `tool_names` 为空、无法调 chem-data-hub 的问题。定向 `tests/test_deliver_mcp.py` + `tests/test_session_events.py`：**8 passed**。
- **D-191（2026-08-27）**：**启动「无模型」假状态**：`model_ready` 改为「任一可运行模型」而非仅默认 Provider；默认不可用时自动切到第一个可用模型；Vite sidecar token 默认目录改为 `%APPDATA%\\ChemClaw`；`getSettings` 非 2xx 不写入 `model_ready`。不改 Provider 存储格式。
- **D-190（2026-08-27）**：**退役内置 chem MCP**。撤销 D-167 产品自动种入：`retire_builtin_mcp()` 在启动时删除 `chemclaw_builtin` 全局条目并清理 `CHEMCLAW_BUILTIN_MCP_*`；Windows 打包不再注入 bundle/templates；`add_mcp`/`patch_mcp`/`delete_mcp` 不再对内置行只读。用户须手动添加 `chem-data-hub` 等 HTTP MCP（Bearer token 写在 `headers` 或 `${VAR}` + `.env`）。D-189 的连接诊断（`misconfigured`/`last_error`/ExceptionGroup 展开）保留。现货路由 D-166 仍依赖用户自配的 chem-data-hub MCP。
- **D-167（2026-08-19；D-189 修订 2026-08-26；D-190 退役 2026-08-27）**：~~内置 chem-data-hub / chem-biz-scope MCP（B1）~~ 已由 D-190 退役。历史：安装包曾携带混淆 bundle 并 seed 全局 `mcp.json`。规格/计划：`docs/superpowers/specs/2026-08-19-chemclaw-builtin-mcp-design.md` / `docs/superpowers/plans/2026-08-19-chemclaw-builtin-mcp.md`。
- **D-189（2026-08-26）**：**内置 MCP 假健康修复**：连接页「内置 · configured」≠会话已连通。修订 D-167 种子策略（产品托管 `.env` 键可刷新）；非 OAuth `ensure` 失败记入 `_mcp_errors` 并 GUI 可见；Authorization 仍含 `${VAR}` 拒连并标 `misconfigured`；已有 TurnEngine 缺 MCP 工具时 `prepare_mcp_tools` 可补挂。**同日补丁**：`format_mcp_connect_error` 递归展开 ExceptionGroup/TaskGroup 叶子异常，GUI 红字为可读根因（鉴权失败/无法连接服务器/证书）而非外壳 TaskGroup 文案。不把 chem-data-hub 打进进程，不改 D-166 路由与 `requires_approval`。
- **D-168（2026-08-19）**：**退役 GUI「客户清单」侧栏页（方案 A）**：删除主导航「客户清单」、`LeadsWorkbench`、JSON 导入/本机标记 UI 与 `requestLeadFollowup` 对话注入桥。保留 `format_lead_list`、`chem-lead-list` 及外贸/内贸拓客龙虾收尾产物（对话 Markdown/CSV）。不删 SMTP/HubSpot 对话 CTA。修订 D-006。计划：`docs/superpowers/plans/2026-08-19-chemclaw-retire-lead-list-nav.md`。
- **D-169（2026-08-20）**：**Agent Harness Phase 1 采用 Scenario/Capability 深模块扩展既有单一执行链路**：`ScenarioResolver -> CapabilityResolver -> TurnPlanner -> Tool Projection -> TurnEngine -> D-166 市场守卫 -> TurnPlan allowlist guard -> PermissionEngine -> ToolRegistry`。内置 Registry 首批只含 5 个 Scenario 与 8 个 Capability；精确 Tool/MCP binding 只存在 Capability Provider 中，Scenario 不复制工具名。Preview 使用真实 Planner 与 live ToolRegistry，但不执行 Tool、不追加消息、不写 Audit/Trace；未连接 MCP 仅报告 configured/unavailable。ToolOutcome 保留 raw payload 给模型，GUI/Trace 只消费标准状态。TurnTrace 与 Audit 分表，只持久化名称、计数、token、耗时、fallback 与最终状态，禁止正文、提示词、参数、结果和推理。`scenario_resolution_enabled` 为独立候选 ON 开关，OFF 恢复 D-165/D-166 行为。Phase 1 不泛化 Subagent、不建 BackgroundTaskManager/Hook Bus/Room/企业微信，也不引入 OpenHarness/DeerFlow/LangGraph/MAF/AutoGen runtime。
- **D-170（2026-08-20）**：**开发诊断能力不占用普通用户界面**：按用户反馈删除账号菜单“活动”入口、App `audit` surface、`AuditView` 与执行诊断前端测试。保留 D-169 的 Scenario/Capability/Guard/ToolOutcome/TurnTrace、Preview 与 Trace API，供内部调试、自动化和后续管理后台使用。产品 UI 不向普通用户暴露工具 allowlist、readiness、route、trace 等开发术语。
- **D-171（2026-08-20）**：**Phase 2 以统一后台任务深模块泛化 Subagent，不新建 Agent Loop**。参考 OpenHarness 的 task lifecycle、DeerFlow 的 executor/cancel/status、OpenAI Agents SDK 的 manager-owned agent-as-tool 与 trace/approval、LangGraph 的 durable interrupt/resume 语义；只吸收合同，不嵌入这些 runtime。`BackgroundTaskManager` 统一 Agent/Shell 的 task id、状态、输出游标、停止、wait/gather、completion listener、重启调和与保留清理；`SubagentRuntime` 通过 `SubagentProfile` 创建仍由既有 `TurnEngine` 驱动的 child session。`explore` 保留前台兼容合同；`research` 只继承声明 MCP；`worker` 可写共享 workspace，但 PermissionEngine/审批仍是最终权威且禁止默认嵌套。Agent 完成后可续发消息并复用持久 child conversation；进程重启把遗留 running/queued 调和为 interrupted，不自动重放 Shell 或 Tool 副作用。Phase 2 只提供 runtime tools 与本地鉴权 REST，不恢复 D-170 用户侧“活动”页；Hook Bus、自动 fallback、Verifier、worktree/remote isolation 与 Room 留到后续独立阶段。
- **D-172（2026-08-20）**：**Phase 2 的产品入口属于当前对话 RightRail，不属于独立“活动/执行诊断”页**。Scenario 只决定 Subagent eligibility 与工具面，模型实际成功调用 `start_subagent` 才创建任务；至少两个独立研究分支时最多并行 3 个 `research` task。`BackgroundTaskManager` 用 `created/status/output` change listener 暴露生命周期，`SessionManager` 只把变化适配为 owner session WS `background_task_changed`；REST/SQLite 仍是刷新、重连和权限隔离权威。RightRail 的 `BackgroundTasksSection` 仅依赖 `sessionId/refreshKey`，任务为空时不渲染；有任务时可看状态、耗时、工具/系统工作记录与结果，可停止或给 Agent 续发要求，Shell 禁止续发。不展示隐藏 reasoning。`research` Profile 增加 `read_file/list_files` 以只读检查会话产物，不扩大写权限。真实会话已验证父轮 Provider 超时不终止已创建 child task。完整 Team/Swarm、Verifier、Hook Bus 与自动切源仍不在本决策范围。
- **D-172 补丁（2026-08-20）**：**研究意图优先于现货/期货查价 Scenario，不打开查价 Scenario 的 Subagent**。含「深度研究/周报/产业链/供需分析」等标记时，ScenarioResolver 在 D-166 spot/futures 适配器之前匹配 `chemical_market_research`（或企业研究）；TurnPlanner 在研究意图下不把 `AGENT`+`legacy_fallback` 降为 `VERIFIED`，从而保留 `subagent_eligible`。纯查价仍走 `cn_futures_market` / `chemical_spot_price` 且 `allow_subagent=false`。`start_subagent` 默认 Profile 为 `research`；代码探索须显式 `profile="explore"`。不占用 Phase 3 Hook Bus 的 D-173。

- **D-174（2026-08-20）**：**Research Subagent 可执行权限 + 可靠停止**：`research` 退出 plan/只读，改为 `interactive` + `shared_workspace`；工具面补齐 `lookup_cn_futures_*`、保证金计算与 `write_file`/`edit_file`/`replace_in_file`，仍禁止嵌套 Subagent；子引擎对 shared-workspace Profile **继承父会话 PermissionEngine.mode**（父为完全访问/`auto` 则子静默放行写报告，父为 `interactive` 则仍走 Inbox）。`TurnEngine.request_interrupt` 跨线程经 owning loop `call_soon_threadsafe` 唤醒；`BackgroundTaskManager.stop` 在短超时后仍非终态时 **强制 cancelled**（`stop requested; force-cancelled`），并用 `_abandoned` 防止晚到的 worker 回写覆盖。`explore` 仍为 plan 只读。不占用 Phase 3 Hook Bus 的 D-173。
- **D-175（2026-08-21）**：**Subagent 自动汇合为主通道、短查为兜底**。对照 Claude Code 完成通知、OpenHarness listener、DeerFlow 终态/短超时、OpenAI Agents SDK fan-in；`DelegationCohortTracker` 按 `(owner_session_id, parent_trace_id)` 登记后台 Agent 批次，全部终态（含 failed/cancelled/interrupted）只 fire 一次，经 `deliver_to_session` 注入汇合消息（idle 新回合 / busy steer）。Agent 终态同时接线 `WakeStore.complete_job`。`background_task_gather` 默认 **60s**，政策禁止长阻塞等待当最终综合。不实现 AgentRoom，不嵌上游 runtime，不占用 D-173。
- **D-176（2026-08-21）**：**live 思考框贴底 + 规划间隙提示**。`.thinking-body` 在用户仍贴底时随 reasoning 滚到最新（框内上翻暂停）；空白期（有 `reasoningStream`、尚无工具/回答）`forceOpen` 保持正文展开，下方轮播「龙虾正在规划下一步」类文案；步骤卡或回答流出现后提示消失。不改 D-070 TurnGroup、settled 思考默认收起、不伪造 reasoning。规格/计划：`docs/superpowers/specs/2026-08-21-chemclaw-thinking-scroll-planning-wait-design.md` / `docs/superpowers/plans/2026-08-21-chemclaw-thinking-scroll-planning-wait.md`。
- **D-177（2026-08-21）**：**期现套利双口径放行**。在 D-166 默认单口径之上，对明确期现对照意图（同句现货+期货、期现/基差、期货+套利/上下游）同一轮放行 chem-data-hub `get_price_trend` 与 `lookup_cn_futures_*`；`MarketIntentKind.CN_SPOT_FUTURES`；禁止交叉替代；**Web/Yahoo 冒充官方价的禁令见 D-181 有序降级修订**（结构化空结果可 Web 补价并标来源，仍禁 Yahoo 报国内期现、禁现货↔期货互顶）。纯「甲醇期货多少钱」/纯现货/裸甲醇澄清不变；区域现货套利无期货限定仍只走现货。规格：`docs/superpowers/specs/2026-08-21-chemclaw-spot-futures-basis-dual-scope-design.md`。
- **D-178（2026-08-21）**：**步骤卡下方 live 思考强制展开**。live ThinkingBlock 只要挂载（`running && reasoningStream && !streaming`）即 `forceOpen`，含工具步骤出现后的新一轮 reasoning remount；贴底跟滚沿用 D-176；TurnGroup 可同时保持展开。规划间隙提示仍仅首包前。不改 settled 默认收起。规格/计划：`docs/superpowers/specs/2026-08-21-chemclaw-live-thinking-open-with-steps-design.md` / `docs/superpowers/plans/2026-08-21-chemclaw-live-thinking-open-with-steps.md`。
- **D-183（2026-08-21）**：**思考结束收起连续可见**。live 结束后 settled「思考过程」标题保留（不整块消失）；`isPlanningWaitWindow` 覆盖 thinking-only 已写入 items、尚无 tool/非空回答的间隙；TurnGroup 上方汇出最新 reasoning 并可展开。D-178 live forceOpen 不变。规格/计划：`docs/superpowers/specs/2026-08-21-chemclaw-thinking-collapse-continuity-design.md` / `docs/superpowers/plans/2026-08-21-chemclaw-thinking-collapse-continuity.md`。
- **D-186（2026-08-21）**：**思考同槽位交接防空白**。补丁 D-183——App 级 `thinkingSlotView` 在 live / 早期 assistant_delta hold / settled 间隙保持同一思考 chrome；纯 live 仍 forceOpen；Turn 已展示 reasoning 时外挂让位。不改 streamGate 词阈。（计划稿曾标 D-184；与研究委派 D-184 冲突后改号。）规格：`docs/superpowers/specs/2026-08-21-chemclaw-thinking-slot-handoff-design.md`。
- **D-179（2026-08-21）**：**研究效率首包硬伤修复**。子智能体继承父会话 `MarketToolSelection`（含 D-177 双口径）；ApiHub generic invalid（Upstream rejected）强制 compact 重试一次并可 EF salvage；`chart_finished_sidecar` 用 `series_name` 避免覆盖 `TOOL_FINISHED.name`；research 补 `grep`；Windows shell 默认 UTF-8；cowork 优先 `grep`/`read_file` 挖中文文本。不做 MCP 超时缓存、全量 tool-trace、wall-time 预算。规格/计划：`docs/superpowers/specs/2026-08-21-chemclaw-research-efficiency-hard-fixes-design.md` / `docs/superpowers/plans/2026-08-21-chemclaw-research-efficiency-hard-fixes.md`。
- **D-181（2026-08-21）**：**化工行情 Web 有序降级**。修订 D-166/D-177：化工现货、国内期货、期现双口径（及对应澄清）默认将 `web_search`/`web_fetch` 并入 `allowed` 且 `web_is_supplemental=True`；prompt 要求先结构化工具，缺失/空结果才可用 Web 补价或补研究知识并标明来源；禁止把 Web 写成 chem-data-hub/交易所官方；禁止现货↔期货交叉替代与 Yahoo 冒充国内期现。A 股/期权仍按 D-152 不改为网页补价。不拆除市场守卫；不做子代理 TTL/行情缓存。规格/计划：`docs/superpowers/specs/2026-08-21-chemclaw-chem-web-ordered-fallback-design.md` / `docs/superpowers/plans/2026-08-21-chemclaw-chem-web-ordered-fallback.md`。
- **D-184（2026-08-21）**：**研究套利强制委派 + LLM 超时可重试**。沥青/甲醇「研究…产业链/上下游/套利」命中 `chemical_market_research` 后，TurnPlanner 将 AGENT 升级为 `DEEP_RESEARCH` 并投影窄父面（web + subagent 控制 + 合成写工具），**不**把行情 MCP/CN 工具留给主代理串行代劳；收紧委派文案要求先 `start_subagent`。`APITimeoutError`/`timed out` 计入流式 transport 重试（无 progress 一次）并给中文「模型接口超时（llm_api）」友好错误。不做 auto-DAG spawn、compaction 错误流、多轮「写」续匹配。规格/计划：`docs/superpowers/specs/2026-08-21-chemclaw-research-delegate-force-design.md` / `docs/superpowers/plans/2026-08-21-chemclaw-research-delegate-force.md`。（计划稿曾标 D-183；与思考收起 D-183 冲突后改号。）
- **D-187（2026-08-21）**：**双模式停止 + 主任务超时可恢复**。用户 GUI/HTTP 手动 `stop(mode=immediate)` 立刻硬停（~3s force-cancel）；智能体 `background_task_stop` 与系统停用 `wrap_up`（先 steer 催写部分报告，~90s 后再 force；Shell 退化为 immediate）。流式 `read` 超时提到 300s；本轮已有工具/部分正文且遇 timeout 时 EF salvage 一次。不做空闲看门狗/硬 TTL 主动杀。规格/计划：`docs/superpowers/specs/2026-08-21-chemclaw-dual-mode-stop-timeout-design.md` / `docs/superpowers/plans/2026-08-21-chemclaw-dual-mode-stop-timeout.md`。（Cursor 计划稿曾标 D-186；与思考槽位 D-186 冲突后改号。）
- **D-188（2026-08-26）**：**Phase 6 企业微信智能机器人 Channel（WebSocket 长连接）**。首版仅智能机器人 API 模式·长连接（无需公网回调）；复用现有 connectors Gateway/`BasePlatformAdapter`/`MessageEvent`，薄层 `coworker/channels/` 仅统一消息模型与映射；连接 id `wecom`，凭据 `bot_id`+`secret` 进 SecretStore；群聊须 @ 才入站；allowlist 对齐 Slack/Telegram；出站经 live WS；Channel 用户不提升本机权限。Donor：LangBot `08307790e55a0d82db073025fd39ca3ee0a8f04c`（仅 adapter 形态）+ `wecom-aibot-sdk>=1.0.8`（并入 `[messaging]`；`[wecom]` 为兼容别名）。不做自建应用 HTTP、飞书/钉钉、第二套 SessionManager、LangBot Agent runtime。规格/计划：`docs/superpowers/specs/2026-08-26-chemclaw-wecom-ai-bot-channel-design.md` / `docs/superpowers/plans/2026-08-26-chemclaw-wecom-ai-bot-channel.md`。
- **D-193（2026-08-31）**：**多平台 Channel 深模块 + 双向文件合同**。在 D-188 之上深化 `coworker/channels`（Envelope/Capabilities/Status/MediaManager）、会话 FIFO（普通 Channel 消息不再 steering 注入）、企微附件与 `send_file`、飞书/钉钉/官方个人微信 iLink（群聊固定不支持）。不嵌入 CowAgent/LangBot runtime；不采用 WeChatFerry。自动契约测试通过；真机验收与安装包另需用户确认。规格/计划：`docs/superpowers/specs/2026-08-31-chemclaw-multichannel-file-transfer-design.md` / `docs/superpowers/plans/2026-08-31-chemclaw-multichannel-file-transfer.md`。
- **D-194（2026-08-31）**：**腾讯云 COS 统一跨终端 FileStorage**。`coworker/filestore` 提供 `FileRef`/`FileStorage`/`TencentCosStorage`；本机对话产物默认本地；仅 Channel `send_file` 按需上传。投递矩阵：Telegram=COS URL；企微原生失败→URL；Slack 等原生优先。公开读 + `folder` 隔离（默认 `chemclaw`）；密钥仅 SecretStore；设置页可配。不做预签名、不做全量自动同步。规格/计划：`docs/superpowers/specs/2026-08-31-chemclaw-cos-file-storage-design.md` / `docs/superpowers/plans/2026-08-31-chemclaw-cos-file-storage.md`。
- **D-195（2026-08-31）**：**企微流式 + 精装 COS HTML（仅 wecom）**。同 `stream.id` 过程刷新草稿/进度，终态默认短总结 + COS 精装报告页链接；MD 确定性 cook（ChartSpec→Chart.js，含滚轮缩放/平移与全屏）；HTML 上传 `ContentDisposition: inline`；用户明确要求气泡全文则不强制链接。不恢复 D-077 模型烹饪；不改飞书/钉钉/微信/GUI。规格/计划：`docs/superpowers/specs/2026-08-31-chemclaw-wecom-stream-report-html-design.md` / `docs/superpowers/plans/2026-08-31-chemclaw-wecom-stream-report-html.md`。
- **D-195b（2026-08-31）**：**精装报告页表格 / Mermaid / 版式纠偏**。`markdown` 库渲染 GFM 表格；`mermaid` fence 走 CDN 真渲染（失败显示浅底源码兜底）；模板升级字体/斑马纹表/移动端横滚/图框样式。仍确定性 cook，不含 KaTeX、不恢复模型烹饪。
- **D-195c（2026-08-31）**：**企微不发 MD 文件**。默认禁止向企业微信 `send_file` 投递 `.md`（自动转精装 HTML）；气泡剥离 `artifact:*.md`；有报告→短总结+HTML 链接；纯问答只回文字、不附文件。
- **D-196（2026-09-01）**：**Channel 人机交互与新会话合同**。企微/个人微信/飞书/钉钉的 `ask_user` 用 Inbox item 作唯一状态源，绑定精确 `platform/account/conversation`；纯文字展示完整问题、选项说明与推荐，支持自然回答、`/answer`、多选和分组逐题。回答绕过普通 FIFO 直接释放挂起轮次；Coordinator 在人工等待期不执行普通 300 秒取消。`/new`=`/reset`：为 Channel target 创建新持久会话并原子切换所有权，旧会话保留，旧 stream/终态不再发回；订阅绑定的桌面群会话禁止群成员重置。普通 Channel 回答不能审批本机目录、计划或外部写入。个人微信 `default` 账号启用账号级 allowlist，扫码后首条入站热刷新授权并保留同一 `context_token`。
- **D-197（2026-09-01）**：**行情意图是引导层，不是执行硬拒绝层**。本决策按用户明确反馈修订 D-166/D-177/D-181 的“执行守卫”部分：Engine 不再返回 `market-scope denied`，行情 turn 也不再被 Scenario allowlist 二次硬拒绝。现货/期货意图、Capability 投影与 prompt 仍引导模型选正确来源；来源标注与禁止交叉冒充仍是回答质量合同，但不再通过这个会产生误杀的守卫强制。PermissionEngine、安全审批与工具自身校验不变。
- **D-198（2026-09-01）**：**Channel 选项必达、个人微信增量流与自然语言新对话**。桌面 WebSocket 与 Channel 不再各自维护 `ask_user` 状态：共享引擎统一创建一个 Inbox item，当前 Channel ContextVar 决定精确镜像目标，桌面可同时展示同一项；发送必须检查 `SendResult` 并在 pending 期间有限重试。个人微信 iLink 以有序分段消息实现增量回复，明确诊断为 `incremental_messages`，不宣称单消息原位更新。四平台对“开新的对话”“新的对话”“新建对话”等有限整句做 `/new` 同义解析；包含其它请求的长句不重置，安全审批规则不变。
- **D-199（2026-09-01）**：**Channel 图表网页交付**。`chart`/`mermaid`/`mmd` 围栏是仅供 ChemClaw 桌面/GUI 渲染的结构化块，不得进入任何普通 Channel 聊天气泡。增量流只发送围栏前与完整围栏后的可读正文；开围栏未闭合时不泄露半段 JSON。终态与模型显式 `send_message` 共用 `rich_output` 清理规则：有渲染块时用 `report_html` cook 生成单文件 HTML，COS 可用优先发公开链接，否则在支持原生文件的 Channel 发 `.html` 附件；Telegram 等无原生文件且未配 COS 时只发清理文字与中文失败说明，绝不以原始 ChartSpec 降级。普通代码围栏（如 Python、JSON 示例）继续原样发送；桌面保存的 assistant message 不改写。规格/计划：`docs/superpowers/specs/2026-09-01-chemclaw-channel-rich-output-design.md` / `docs/superpowers/plans/2026-09-01-chemclaw-channel-rich-output.md`。
- **D-199b（2026-09-01）**：**Channel 图表预览图 + 统一 HTML 来源**。终态/`send_message` 在 HTML 链接之外 best-effort 发送与链接**同源**的 PNG 预览（Playwright 截取首个 `.chart-figure`）；`report.md` 优先于正文 inline chart 作为 cook 来源，只生成**一份** HTML、只发**一条**链接（完整版 vs 交互图表）。四平台共用 `compose_channel_rich_reply`，不再与企微 `compose_wecom_final_reply` 互斥分叉。精装 HTML 移动端增加 chart-wrap 高度与默认 X 轴 zoom（最新 24 根）。预览失败不阻断 HTML 链接；仍不回退原始 JSON。
- **D-200（2026-09-01）**：**个人微信中途过程流**。在 D-198 多气泡增量之上，无正文草稿时对 `tool_started` 发送「ChemClaw 正在调用 {tool}…」过程气泡（≥2s 节流、同文案不重复）；开轮 ack / 错误用 force 不占用工具节流；`WEIXIN_STREAM_MIN_CHARS` 降至 80；错误发中文气泡。仍为 `incremental_messages`，不宣称 iLink 原位刷新。规格/计划：`docs/superpowers/specs/2026-09-01-chemclaw-weixin-midturn-progress-design.md` / `docs/superpowers/plans/2026-09-01-chemclaw-weixin-midturn-progress.md`。
- **D-201（2026-09-02）**：**快速问答与分阶段市场报告**。`source` 只服务展示，`TurnOrigin` 决定保守执行：桌面和普通 Channel 用户入站均为 `user`，恢复/定时/自唤醒/后台仍保守。周报、日报、月报确定性进入 `chemical_market_report`，前台受限工具面三轮内先答摘要，后台 `market_report` 任务补全 `report.md` 并复用 HTML/PNG 交付。行情大回包由确定性聚合器压缩，禁止让模型通过 shell/脚本解析。报告 180 秒收尾、240 秒强停；Channel 总超时先给收尾宽限并保证终态；个人微信仍为节流增量进度，不能伪装原位编辑。规格/计划：`docs/superpowers/specs/2026-09-02-chemclaw-fast-report-design.md` / `docs/superpowers/plans/2026-09-02-chemclaw-fast-report.md`。
- **D-202（2026-09-02）**：**Channel 快速交付面**。所有带 `source.target` 的 IM Channel（企微/个人微信/飞书/钉钉/Telegram/Slack）用户轮：禁止 shell/load_skill/subagent/todo；AGENT/DEEP_RESEARCH 降级为 VERIFIED 窄面；周报在 Channel 上单轮 `REPORT_CHANNEL`（允许 write_file、6 轮内写 report.md），不启 D-201 后台子任务、不向 IM 刷进度气泡。引擎层拒绝 Channel 上 run_shell 与 outbound-clip 大文件读取；chem-data-hub 价格工具回包经 `MarketSeriesAggregator` 预聚合；液化气/LPG 等别名映射规范品名。终态仍走 D-195/D-199 `rich_output` HTML/PNG。桌面 GUI 保留 D-201 后台补全路径。
