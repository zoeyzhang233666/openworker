...
- **上游 Wave B（2026-08-11，D-125）**：已合入 `chemclaw-clean`（fast-forward）。OpenWorker [#471](https://github.com/andrewyng/openworker/pull/471) ask_user 2.0：富选项（说明/推荐/预览）+ 最多 4 题分组 stepper；旧单题/字符串 options/Inbox JSON 兼容；GUI 全 i18n。验证：`pytest` ask+durable **15 passed**；`tsc` OK；`ask-upgrades` e2e **2 passed**。Wave C Memory（#472）未做，须点名。
...
- 当前分支：`chemclaw-clean`（Wave A+B 已合入；相对 origin ahead；push 须点名）
