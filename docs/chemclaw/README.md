...
- **D-205：OpenCode Go 稳定会话 Header（2026-09-09）**：OpenAI 自定义端点精确命中 `opencode.ai/zen/go/v1` 时，复用 `SessionManager → build_engine → audit_context` 的 ChemClaw `session_id`，由 Chat Completions provider 以 per-request `extra_headers` 注入 `x-opencode-session`；stream、complete 及 transport fallback 同值，内部参数不进入 API body，其他 provider/兼容端点不受影响。provider/router/engine 定向回归：**124 passed**。规格/计划：[`design`](../superpowers/specs/2026-09-09-chemclaw-opencode-go-session-design.md) / [`plan`](../superpowers/plans/2026-09-09-chemclaw-opencode-go-session.md)。开发 sidecar 须重启后生效。
- **场景体系与 Prompt 总纲（2026-09-03）**：已从真实用户任务重构 L0–L5 场景树，正式 L4 共 **37 个**（内贸 16、外贸 9、产业研究 7、期现风险 5）；每个 L4 均含 16 项场景合同、A–K Prompt 合同、五类数据与真实性/审批边界。1.0 建议收敛为 8 个真实任务 Demo：潜客商机、关键联系人、采购需求、智能寻源、到厂成本、企业尽调、现货解释和海外买家。成稿：[`ChemClaw-场景体系与Prompt思路.md`](ChemClaw-场景体系与Prompt思路.md)。本文为待专家/客户验证的产品规划，**未改运行时代码、未表示功能已实现**。
- **D-204：裸化工查价默认现货 MCP + 快答（2026-09-02）**：无期现歧义的「柠檬酸价格/走势图/今天多少钱」等不再判 `non_market`（此前 Channel VERIFIED 只剩网页，模型会连环 ask_user 并声称不能调 MCP）。默认投影 `get_price_trend`；甲醇/原油等期现歧义仍澄清。Prompt/企微约定：有 MCP 立即查，禁止「请你贴生意社数据」问卷；只要今天/现价用小 limit 短答，要走势图再拉序列。定向 `test_market_intent` + `test_channel_fast_surface_d202` + projection/scenario/planner/wecom guidance：**78 passed**。**须重启 sidecar** 后企微复验「柠檬酸价格走势图」「柠檬酸今天多少钱」。
- **D-203：Channel 图表 PNG 改用 Matplotlib（2026-09-02）**：企微等平台预览图不再依赖 Playwright/Chromium，也不用手搓 Pillow 位图。`render_chart_spec_png` 用 Matplotlib Agg + 系统中文字体（微软雅黑等）直接从 ChartSpec 出 PNG，中文标题/图例/坐标可读。`pyproject.toml` `[messaging]` 增加 `matplotlib`；`packaging/openworker-server.spec` **取消**对 `matplotlib` 的 excludes，并 `collect_all(matplotlib)` 打进 sidecar，保证新机器安装包无需本机 pip。Playwright 仍仅属 `[browser]` 可选。定向 `test_channel_rich_output_d199`：**14 passed**。开发态须重启 sidecar；正式安装包须用户确认后重打 NSIS。
- **D-202：Channel 快速交付面（2026-09-02）**：企微/个人微信/飞书/钉钉/Telegram/Slack 等 Channel 用户轮默认窄工具面（禁 shell/load_skill/subagent）；周报/日报/月报命中 `chemical_market_report` 时走 `REPORT_CHANNEL` 单轮写 `report.md` + 终态 HTML/PNG（不再启 D-201 后台子任务与 IM 进度气泡）。引擎对 chem-data-hub 价格回包做 `MarketSeriesAggregator` 预聚合；液化气/LPG 等别名确定性映射为「液化石油气」。桌面 GUI 仍可选 D-201 分阶段后台补全。定向 `test_channel_fast_surface_d202` + `test_turn_planner` + `test_market_series_d201`：**24 passed**。**须重启 sidecar** 后企微/个微复验液化气周报（目标 ≤6 工具、有 HTML 链接、无进度垃圾消息）。
- **D-201：快速问答与分阶段市场报告（2026-09-02）**：普通桌面/Channel 用户消息不再因 `source` 展示卡片进入全工具 Agent；周报/日报/月报确定性命中受限 `chemical_market_report`，三轮内先答摘要，后台生成 `report.md` 并沿用 HTML/PNG/文件交付。新增行情聚合器以替代 Shell 解析大回包；报告 180 秒请求收尾、240 秒强停，Channel 总超时有 15 秒收尾宽限和终态保障。个人微信保持节流增量进度，企微优先原位流更新。规格/计划：[`design`](../superpowers/specs/2026-09-02-chemclaw-fast-report-design.md) / [`plan`](../superpowers/plans/2026-09-02-chemclaw-fast-report.md)。**Channel 单轮交付见 D-202**；桌面后台路径仍保留。
- **D-200：个人微信中途过程流（2026-09-01）**：工具轮次在最终回答前发送「正在调用 {tool}…」多气泡过程提示（≥2s 节流）；正文分段门槛降至 80 字；错误发中文气泡；仍为 `incremental_messages`，不假装企微原位刷新。D-200 定向（含 tool/error）+ D-196/D-198/D-199/企微流/多平台组合回归：**94 passed, 1 skipped**。**须重启 sidecar** 后个人微信真机复验过程提示。规格/计划：[`design`](../superpowers/specs/2026-09-01-chemclaw-weixin-midturn-progress-design.md) / [`plan`](../superpowers/plans/2026-09-01-chemclaw-weixin-midturn-progress.md)。
- **D-199c：精装 HTML GFM 表格修复（2026-09-01）**：助手正文 cook 为 Channel HTML 时，pipe 表格若紧跟标题/段落、缺少空行，会被 nl2br 包进 `<p>` 导致 `| 列 |` 裸显。`md_html.normalize_gfm_tables` 在转换前隔离表格块；所有 `cook_report_html` 路径（交互图表、完整报告、企微 md→html）共用。**须重启 sidecar** 后新生成的 COS 链接才生效。
- **D-199b：Channel 图表预览图 + 移动网页修复（2026-09-01）**：在 D-199 基础上，Channel 终态/`send_message` 对含 chart 的 HTML **同时发送 PNG 预览图（聊天气泡内嵌）与单一 HTML 链接**；预览与链接**同源**（同一份 cook 结果）。workspace 存在 report.md 时**优先 cook 完整报告**（链接「查看完整版」），否则 cook 正文 inline chart（「查看交互图表」）；统一合成器取代 D-199/D-195 互斥分叉。精装 HTML 移动端正文：chart 默认 zoom 最新 24 根 + 更高 chart-wrap。D-199b 定向 + 组合回归：**174 passed, 1 skipped**。**须重启 sidecar** 后企微复验预览图 + 链接。
- **D-199：Channel 图表网页交付（2026-09-01）**：企微、个人微信、飞书、钉钉、Telegram、Slack 等无前端渲染能力的 Channel 不再把 ` ```chart` / Mermaid 原始 JSON 发进聊天气泡。`coworker/channels/rich_output.py` 统一过滤流式可见文本，并在终态/`send_message` 用现有 `report_html` cook 生成精装 HTML，经 COS 链接或 `.html` 附件交付；上传失败只发清理后的文字与中文降级说明，绝不回退原始数据。桌面 assistant message 不改写。D-199 定向 + D-195/D-196/D-198/连接器组合回归：**171 passed, 1 skipped**。**须重启 sidecar** 后企微/个人微信复验含 chart 的回答。规格/计划：[`design`](../superpowers/specs/2026-09-01-chemclaw-channel-rich-output-design.md) / [`plan`](../superpowers/plans/2026-09-01-chemclaw-channel-rich-output.md)。
- **D-198：Channel 选项必达、个人微信增量流与自然语言新对话（2026-09-01）**：修复桌面 WebSocket 覆盖共享 `question_asker` 后，企微/个人微信只显示“正在处理”却收不到选项的真实回归；同一 Inbox item 现在可同时显示在桌面与精确发起会话，真实发送失败会有限重试。个人微信基于官方 iLink 能力提供“处理提示 + 节流分段 + 终态尾段去重”的增量回复（不是伪造单气泡原位编辑）。四平台新增“开新的对话”“新的对话”“新建对话”等整句自然语言重置，仍复用 `/new` 的原子切换和旧输出抑制。D-193/D-195/D-197 组合回归：**170 passed, 1 skipped**；D-198 单文件：**31 passed**；桌面 WS/Inbox/ask_user 补充回归：**34 passed, 1 deselected**（旧行情矩阵会访问真实新浪期货接口，未纳入离线断言）；compileall 通过。开发 sidecar/Vite 已从当前 worktree 隐藏重启：health=ok、Vite=200；企微与个人微信均 connected/authenticated、queue=0、无 runtime error，个人微信报告 `incremental_messages`。规格/计划：[`design`](../superpowers/specs/2026-09-01-chemclaw-channel-prompt-weixin-stream-natural-reset-design.md) / [`plan`](../superpowers/plans/2026-09-01-chemclaw-channel-prompt-weixin-stream-natural-reset.md)。真实账号复验仍待用户触发。
- **D-196：Channel 交互闭环与会话重置（2026-09-01）**：企业微信、个人微信、飞书、钉钉的 `ask_user` 问题会回到精确发起会话，支持序号/选项文字/`/answer`/多选/分组逐题；无效或跨会话回答不误开新轮次，安全审批仍只能在电脑端。四平台私聊及自动管理群支持精确 `/new`/`/reset`，旧会话保留，旧轮次不再串回 Channel；个人微信 `default` 账号按账号 allowlist 授权，扫码后首条入站会热刷新授权，并增加不含正文/凭据的 poll 诊断。
- **D-197：取消行情执行硬守卫（2026-09-01）**：按用户实施中修订，Engine 不再以 `market-scope denied` 或行情 Scenario allowlist 拒绝 MCP；市场意图、工具投影和 prompt 只负责引导正确现货/期货来源，PermissionEngine 与工具自身校验仍是安全边界。包含 D-196/D-197 及 D-192/D-193/D-195 的完整定向回归：**166 passed, 1 skipped**（可写 basetemp）；Python `compileall` 通过。开发 sidecar/Vite 已重启，health=ok，企微与个人微信均 connected/authenticated、queue=0、无 runtime error；个人微信最近 poll messages=0，待真实入站。规格/计划：[`design`](../superpowers/specs/2026-09-01-chemclaw-channel-interaction-reset-market-design.md) / [`plan`](../superpowers/plans/2026-09-01-chemclaw-channel-interaction-reset-market.md)。真机清单：[`channel_d196_acceptance.md`](fixtures/channel_d196_acceptance.md)。
- **D-195c：企微不发 MD（2026-08-31）**：企微默认不投递 Markdown 文件（`send_file` 遇 `.md` 自动转精装 HTML）；气泡去掉 `artifact:*.md`；有报告才附 HTML 完整版链接；纯问答只回文字。定向 `test_report_html_cook` + `test_send_file` + `test_wecom_stream_reply`：**27 passed**。**须重启 sidecar**。
- **D-195b：精装报告页表格 / Mermaid / 版式（2026-08-31）**：`markdown` 库修 GFM 表格裸显；Mermaid CDN 真渲染；报告页字体/斑马纹表/横滚/图框样式升级。定向 `test_report_html_cook` + `test_wecom_stream_reply`：**17 passed**。须重启 sidecar 后重新问企微生成完整版链接（旧 COS HTML 不会自动刷新）。
- **D-195：企微流式 + 精装 COS HTML（2026-08-31）**：仅 `wecom`——同 stream 过程刷新后收束为短总结 + COS 精装 HTML（MD cook + Chart.js 直出图，含缩放/平移/全屏；`inline` 打开）。COS 未配置则中文降级提示。飞书/钉钉/微信/GUI 默认不变。定向 `test_report_html_cook` + `test_wecom_stream_reply` + `test_wecom_channel` + `test_filestore_d194`：**52 passed**。真机清单：[`wecom_d195_acceptance.md`](fixtures/wecom_d195_acceptance.md)。规格/计划：[`D-195 design`](../superpowers/specs/2026-08-31-chemclaw-wecom-stream-report-html-design.md) / [`plan`](../superpowers/plans/2026-08-31-chemclaw-wecom-stream-report-html.md)。**须重启 sidecar** 后企微验收过程刷新与完整版链接。
...
- **本机 NSIS 安装包（2026-08-31 重打）**：用户点名自本 worktree `chemclaw-UI` @ `0a66895`（含 D-193/D-194 多平台 Channel + COS FileStorage）打未签名包；产物 `ChemClaw_0.1.7_x64-setup.exe`（约 **80.1 MiB** / 80.07 MB）于 `surfaces/gui/src-tauri/target/release/bundle/nsis/`。无 updater 签名密钥，不含自动更新制品。staged sidecar 验收：**108** `SKILL.md` + **7** builtin persona（含销售四龙虾 / `platform-rewrite-lobster` / `chain-lobster` / `ops`）；**无** `builtin_mcp.bundle`（D-190）；sidecar 内无 `sessions`/transcripts。聊天只在 `%APPDATA%\\ChemClaw`，**不进安装包**；若安装后仍见旧聊天/旧技能，请先改名备份该目录再首次启动以拿到本树最新 seed。
