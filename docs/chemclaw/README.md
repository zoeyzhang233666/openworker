...
- **本机 NSIS 安装包（2026-08-19 重打）**：用户点名自本 worktree `chemclaw-clean` @ `ad63ab9` + 未提交工作区（含 D-161—D-164 / CN 行情后续与 ChartBlock bar|scatter 类型收窄最小修复）重打；未签名 `ChemClaw_0.1.7_x64-setup.exe`（约 **49.7 MiB** / 52.1 MB，mtime 09:22）于 `surfaces/gui/src-tauri/target/release/bundle/nsis/`。无 updater 签名密钥，不含自动更新制品。staged sidecar 验收：**108** `SKILL.md` + **7** builtin persona（含 `export-sales-lobster` / `platform-rewrite-lobster`）；sidecar 内无 `sessions`/transcripts 目录。聊天只在 `%APPDATA%\\ChemClaw`，不进安装包；要无旧聊天+最新 seed 技能请先改名备份该目录再首次启动。
