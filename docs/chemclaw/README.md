...
- 销售增长智能：外贸/内贸/商机/转化设计已于 2026-08-07 获批。**已授权并实现（D-091—D-103、D-105—D-111）**：四龙虾包；PubChem/GLEIF/国内登记；TED；SAM.gov；Comtrade；海关 **CSV/XLSX** 筛选；询盘转报价；客户清单首包+进阶 UX；SMTP 发送审批；HubSpot CRM **笔记 + 创建联系人**审批 CTA；**化工社合集单包试点** `uncertainty-and-units`（可选挂询盘/转化，不进 Lead 评分）。默认仍为 ChemClaw/`cowork`，销售 Agent 默认禁用；清单页无发送/CRM 按钮；草稿 ≠ 发送；不自动写 CRM。**本机销售环点检（2026-08-09）用户确认通过**。
...
4. 销售主线含 D-105/D-109 HubSpot、D-106 SAM、D-108/D-110 海关 CSV/XLSX 已收口；内容重构 M1+M2（D-104/D-107）已落地；**D-111** 合集单包 `uncertainty-and-units` 已试点。**下一小任务（须点名）**：内容重构 **M3**（须确认打安装包）；下一化工社合集包；CRM 字段/任务 CTA；或海关外部 API。不批量内置化工社 Skills，不自动外发/写 CRM。

## 文档索引

- [完整产品设计](../superpowers/specs/2026-07-29-chemclaw-product-design.md)
- [销售增长智能设计（D-086—D-111）](../superpowers/specs/2026-08-07-chemclaw-sales-growth-intelligence-design.md)
