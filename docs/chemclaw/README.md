...
- **上游 Wave C（2026-08-11，D-126）**：在分支 `chore/upstream-memory-2026-08-11` 完成 OpenWorker [#472](https://github.com/andrewyng/openworker/pull/472) Memory V1 四段移植（core → settings/REST → GUI/Undo → CLI/TUI）。语义：关=停写入；已有记忆仍可读/注入新会话；写开关 live；user rules 会话启动时固定。**合回 `chemclaw-clean` 须点名**。验证：`pytest` memory+api **52 passed**；GUI `tsc` OK；Transcript/i18n/audit **55 passed**；`python -m coworker.cli --help` OK。
...
- 当前分支：`chemclaw-clean`（Wave A+B 已合入；Wave C 在 `chore/upstream-memory-2026-08-11`，合回须点名；相对 origin ahead；push 须点名）
