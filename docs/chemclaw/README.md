...
- 销售增长智能：外贸/内贸/商机/转化设计已于 2026-08-07 获批。**已授权并实现（D-091—D-103、D-105—D-108）**：四龙虾包；PubChem/GLEIF/国内登记；TED；SAM.gov；Comtrade；**海关/提单 CSV 企业级筛选**；询盘转报价；客户清单首包+进阶 UX；SMTP 发送审批；HubSpot CRM 笔记写入审批。默认仍为 ChemClaw/`cowork`，销售 Agent 默认禁用；清单页无发送/CRM 按钮；草稿 ≠ 发送；不自动写 CRM。**本机销售环点检（2026-08-09）用户确认通过**。
...
4. 销售主线含 D-105 HubSpot、D-106 SAM、D-108 海关 CSV 筛选已收口；内容重构 M1+M2（D-104/D-107）已落地。**下一小任务（须点名）**：内容重构 **M3**（须确认打安装包）；化工社单包；CRM 扩展写入面；或海关 XLSX/外部 API。不批量内置化工社 Skills，不自动外发/写 CRM。

## 文档索引

- [完整产品设计](../superpowers/specs/2026-07-29-chemclaw-product-design.md)
- [销售增长智能设计（D-086—D-108）](../superpowers/specs/2026-08-07-chemclaw-sales-growth-intelligence-design.md)
