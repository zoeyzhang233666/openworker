...
- **D-194：腾讯云 COS 统一 FileStorage（2026-08-31）**：平台无关 `FileRef`/`FileStorage`；本机 EXE 产物默认只写 workspace、不上云；仅 `send_file` 向 Channel 投递时按需上传。Telegram 以 COS 公开 URL 交付；企微原生失败则 URL 降级；Slack 等原生优先。设置页「云文件存储」配置桶/URL/folder，密钥进 `SecretStore`（`filestore:cos`）。定向 `test_filestore_d194` + `test_send_file` + `test_multichannel_d193` + `test_wecom_channel`：**54 passed, 1 skipped**；GUI `ChannelConnectorStatus`：**5 passed**。规格/计划：[`D-194 design`](../superpowers/specs/2026-08-31-chemclaw-cos-file-storage-design.md) / [`plan`](../superpowers/plans/2026-08-31-chemclaw-cos-file-storage.md)。
- **个人微信扫码 UX 纠偏（2026-08-31）**：连接页改为 CowAgent 式「一键出码」——`WeixinDetail`/`WeixinAvailable` 大二维码居中；列表/弹窗不再默认露出 Token 表单；`auth_required` 不再被 `start()` 误标为已连接；半连接状态自动拉码；旧版把「等待扫码」文案当 account id 的 profile 启动时迁移到 `default`。**须重启 sidecar + 重载 GUI** 后验收：连接 → 个人微信 → 点「扫码连接」或进入详情应直接见二维码。
- **D-193：多平台 Channel 与文件传输（2026-08-31）**：统一 `coworker/channels` Envelope/Capabilities/媒体安全；`ChannelDeliveryCoordinator` 按会话 FIFO；企微补强 + 飞书/钉钉/官方个人微信 iLink 适配器；`send_file` 注册四平台；GUI 只展示真实 capabilities/扫码状态。个人微信固定不支持群聊。定向 `tests/test_multichannel_d193.py` + `tests/test_wecom_channel.py`：**37 passed, 1 skipped**；GUI `ChannelConnectorStatus`：**2 passed**。真机租户验收与正式安装包仍待用户确认。规格/计划：[`D-193 design`](../superpowers/specs/2026-08-31-chemclaw-multichannel-file-transfer-design.md) / [`plan`](../superpowers/plans/2026-08-31-chemclaw-multichannel-file-transfer.md)。
- **本机 NSIS 安装包（2026-08-28 10:39 重打）**：用户点名自本 worktree `chemclaw-UI` @ `aa3aac0` 打未签名包；产物 `ChemClaw_0.1.7_x64-setup.exe`（约 **65.8 MiB** / 68.99 MB，mtime 10:39）于 `surfaces/gui/src-tauri/target/release/bundle/nsis/`。无 updater 签名密钥，不含自动更新制品。staged sidecar 验收：**108** `SKILL.md` + **7** builtin persona（含销售四龙虾 / `platform-rewrite-lobster` / `chain-lobster` / `ops`）；**无** `builtin_mcp.bundle`（D-190）；sidecar 内无 `sessions`/transcripts。聊天只在 `%APPDATA%\\ChemClaw`，**不进安装包**；若安装后仍见旧聊天/旧技能，请先改名备份该目录再首次启动以拿到本树最新 seed。
- **D-192：Channel 后台投递补挂 MCP（2026-08-27）**：企业微信/Slack/Telegram 等 Channel 消息经 `deliver_to_session` 投递时此前不调用 `prepare_mcp_tools`，会话引擎只有「已配置但未连接」的 Harness 标记、无实际 `mcp__*` 工具；GUI WebSocket 路径正常。现新增 `_prepare_engine_for_turn()`，后台轮次与 GUI 一致先连 MCP 再 `get_engine`。定向 `tests/test_deliver_mcp.py` + `tests/test_session_events.py`：**8 passed**。**须重启 sidecar** 后企微再问「mcp 是否正常」应能调 chem-data-hub 工具。
- **D-191：启动「无模型」假状态修复（2026-08-27）**：默认模型的 Provider 无 key、但其它 Provider 已配置时，`model_ready` 曾为 false，Composer 用「无模型」挡住整个下拉框，只能靠再贴一次 API key（`set_provider` 顺带改默认）才恢复。现改为：有任一可运行模型则 `model_ready=true`，并自动把默认切到第一个可用模型；Vite 开发 token 路径对齐 ChemClaw（不再读遗留 `APPDATA\\coworker`）；`getSettings` 非 2xx 不污染状态。定向 `test_provider_router` + `test_settings`：**10 passed**。**须重启前后端**（Vite 需重载 token 路径）。
- **D-190：退役内置 chem MCP（2026-08-27）**：不再种入 `chem-data-hub` / `chem-biz-scope`；`SessionManager` 启动时 `retire_builtin_mcp()` 删除 `mcp.json` 中带 `chemclaw_builtin` 的旧条目并清理 `.env` 里 `CHEMCLAW_BUILTIN_MCP_*`；打包不再携带 `builtin_mcp.bundle` / `builtin_servers.json`。用户须在 **连接 → MCP → + 添加服务器** 自行粘贴配置（与 Cursor 相同：`url` + `headers.Authorization: Bearer <token>`）。`add_mcp` 剥离伪造的 builtin 标记；所有 MCP 可删可改。定向 `tests/test_builtin_mcp.py` + `test_mcp*.py`：**25 passed**；GUI `ManageTabs.mcpBuiltin`：**1 passed**。**须重启 sidecar** 后连接页不再出现产品内置行。
- **D-189：内置 MCP 假健康修复（2026-08-26；D-190 后仅保留连接诊断能力）**：`prepare_mcp_tools` 连接失败写入 `last_error`；未解析 `${VAR}` / 近期失败显示 `misconfigured`（配置不完整）；引擎已缓存但缺 `mcp__` 时可补挂。`format_mcp_connect_error` 展开 ExceptionGroup/TaskGroup 叶子异常并加中文前缀。
- **D-188：企业微信智能机器人连接（2026-08-26）**：Phase 6 首版——企微智能机器人 WebSocket 长连接（`wecom` connector）；`coworker/channels` 薄映射层；Gateway/`WecomBotAdapter`；连接设置可连可断；群 @ / 私聊；allowlist；出站 live WS。**SDK 随 `[messaging]`（`wecom-aibot-sdk>=1.0.8`；`[wecom]` 为别名）**；开发 venv / Windows 打包需带 messaging。不做自建应用回调/飞书/钉钉。定向 `tests/test_wecom_channel.py` **22 passed**；connectors 子集 **4 passed**；GUI `ConnectorIcon` **6 passed**。真人清单：[`wecom_ai_bot_acceptance.md`](fixtures/wecom_ai_bot_acceptance.md)。规格/计划：[`D-188 design`](../superpowers/specs/2026-08-26-chemclaw-wecom-ai-bot-channel-design.md) / [`plan`](../superpowers/plans/2026-08-26-chemclaw-wecom-ai-bot-channel.md)。
...
- **内置 chem-data-hub / chem-biz-scope MCP（2026-08-19，D-167；D-189 修订；D-190 退役）**：**D-190** 起产品不再自动种入；历史 bundle/seed 逻辑仅代码库保留供参考。用户自行在连接页添加 MCP。规格/计划：[`D-167 design`](../superpowers/specs/2026-08-19-chemclaw-builtin-mcp-design.md) / [`plan`](../superpowers/plans/2026-08-19-chemclaw-builtin-mcp.md)。
